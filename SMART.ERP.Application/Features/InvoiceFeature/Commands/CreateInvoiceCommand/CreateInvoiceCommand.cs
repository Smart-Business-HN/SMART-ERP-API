    ...
    public class CreateInvoiceCommandHandler : IRequestHandler<CreateInvoiceCommand, Response<InvoiceDto>>
    {
        ...
        public async Task<Response<InvoiceDto>> Handle(CreateInvoiceCommand request, CancellationToken cancellationToken)
        {
            ...
            await _unitOfWork.ExecuteInTransactionAsync(async ct =>
            {
                ...
                if (newRecord.IsDeferredRevenue)
                {
                    // Devengo mensual: arranca el día de facturación; el job corre cada mes en esa misma fecha.
                ...
                {
                    newRecord.RecognitionMonths = null;
                    newRecord.RecognitionStartDate = null;
                    newRecord.MonthsRecognized = 0;
                    newRecord.NextRecognitionDate = null;
                }
                ...
                await RegisterKardexSaleMovements(productsForNewInvoice, request.BranchOfficeId, invoiceResponse, customerExist?.FullName, KardexMovementType.Sale, ct);
        ...
        /// ProductId (texto libre) y los productos tipo Servicio no afectan inventario y se omiten.
        /// Los combos se expanden en movimientos por componente según su receta. No valida stock:
