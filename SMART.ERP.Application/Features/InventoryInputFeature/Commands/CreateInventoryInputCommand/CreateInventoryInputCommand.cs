...
namespace SMART.ERP.Application.Features.InventoryInputFeature.Commands.CreateInventoryInputCommand
{
    public class CreateInventoryInputCommand : IRequest<Response<InventoryInputDto>>
    {
        ...
        public List<CreateProductEntryDto>? ProductsToEntries { get; set; }
    }
    public class CreateInventoyInputCommandHandler : IRequestHandler<CreateInventoryInputCommand, Response<InventoryInputDto>>
    {
        ...
        public async Task<Response<InventoryInputDto>> Handle(CreateInventoryInputCommand request, CancellationToken cancellationToken)
        {
            ...
            if (request.ProductsToEntries != null)
            {
                var checkProducts = await CheckMasterDataOfProductToInput(request.ProductsToEntries);
            ...
            if (request.ProductsToEntries != null && request.ProductsToEntries.Count > 0)
            {
                foreach (var item in request.ProductsToEntries)
                {
            ...
            inventoryEntryResponse.ProductEntries = await _productEntryRepositoryAsync.ListAsync(new ProductEntrySpecification(inventoryEntryResponse.Id));
