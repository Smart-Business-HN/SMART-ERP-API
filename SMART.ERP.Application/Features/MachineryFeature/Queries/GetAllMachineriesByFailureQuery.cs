...
{
    public class GetAllMachineriesByFailureQuery : IRequest<Response<List<MachineriesByFailureDto>>>
    {
    ...
    public class GetAllMachineriesByFailureQueryHandler : IRequestHandler<GetAllMachineriesByFailureQuery, Response<List<MachineriesByFailureDto>>>
    {
        ...
        public GetAllMachineriesByFailureQueryHandler(IRepositoryAsync<Machinery> repositoryAsync,
        ...
        public async Task<Response<List<MachineriesByFailureDto>>> Handle(GetAllMachineriesByFailureQuery request, CancellationToken cancellationToken)
        {
            ...
            var machineries = await _repositoryAsync.ListAsync(new GetMachineriesByStatusSpecification(request.SubcategoryId, countryName, request.BrandId, request.Status));
            if (provincies.Count > 0)
            {
                ...
                response = response.Where(a => a.MachineryFailureReports.LastOrDefault(x => x.MachineryFailure.Name != "Sin falla") != null).ToList();
            }
            else
            {
                response = machineries.Where(a => a.MachineryFailureReports.LastOrDefault(x => x.MachineryFailure.Name != "Sin falla") != null).ToList();
            }

            var group = response.GroupBy(a => a.MachineryFailureReports.LastOrDefault().MachineryFailure.Name).Select(a => new MachineriesByFailureDto
            {
                Failure = a.Key,
                Quantity = a.Count(),
                Machineries = a.Select(sm => new MachineryMissingMaintenanceDto
                {
                    ...
                    Failure = sm.MachineryFailureReports.First().Description,
                    CreationDateFailure = sm.MachineryFailureReports.First().CreationDate
                }).ToList()
            }).ToList();
            return new Response<List<MachineriesByFailureDto>>(group);
        }
    }
}
