...
{
    public class GetMachineriesByStatusQuery : IRequest<Response<List<MachineriesByStatusDto>>>
    {
    ...
    public class GetMachineriesByStatusQueryHandler : IRequestHandler<GetMachineriesByStatusQuery, Response<List<MachineriesByStatusDto>>>
    {
        ...
        public GetMachineriesByStatusQueryHandler(IRepositoryAsync<Machinery> repositoryAsync,
        ...
        public async Task<Response<List<MachineriesByStatusDto>>> Handle(GetMachineriesByStatusQuery request, CancellationToken cancellationToken)
        {
            ...
            if (provincies.Count > 0)
            {
                ...
                response = response.Where(a => a.MachineryFailureReports.Count > 0).ToList();
            }
            else
            {
                response = machineries.Where(a => a.MachineryFailureReports.Count > 0).ToList();
            }

            var group = response.GroupBy(a => a.MachineryFailureReports.LastOrDefault().Status.Name).Select(a => new MachineriesByStatusDto
            {
                Status = a.Key,
                Quantity = a.Count(),
                Machineries = a.Select(sm => new MachineryMissingMaintenanceDto
                {
                    ...
                    Failure = sm.MachineryFailureReports.First().MachineryFailure.Name,
            ...
            return new Response<List<MachineriesByStatusDto>>(group);
        }
    }
}
